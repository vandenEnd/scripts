"""
GPR shock propagation and AI-exposure mitigation: NL + euro area panel
========================================================================
...
effects only (no time effects):
  ...
  Patent spec / Investment spec (identical structure, different F):
    Delta_gdp[i,t+h] = a[i,h] + b1_h*GPR_shock_t + b2_h*F[i,t]
                       + b3_h*(GPR_shock_t * F[i,t]) + Gamma_h*X[i,t]
                       + e[i,t+h]

  Corr spec (Dum = is_boom_t; Dum=0/"bust" is the reference level):
    Delta_gdp[i,t+h] = a[i,h] + b1_h*GPR_shock_t + b2_h*F[i,t]
                       + b3_h*Dum_t + b4_h*(GPR_shock_t*F[i,t])
                       + b5_h*(GPR_shock_t*Dum_t)
                       + b6_h*(GPR_shock_t*F[i,t]*Dum_t)
                       + Gamma_h*X[i,t] + e[i,t+h]

for h = 0..8 quarters, where F[i,t] in (0,1) is a logistic transform of a
country-level AI-exposure state variable z[i,t], and GPR_shock_t is
an AR(2)-purified version of the global Geopolitical Risk Index
(Caldara & Iacoviello; see purify_gpr_shock()) -- the residual
after regressing GPR on its own lags, an approximate "identified shock"
rather than the raw level.

IDENTIFICATION: GPR_shock_t, Dum_t, and GPR_shock_t*Dum_t are
entity-invariant (identical across every country at a given quarter --
GPR is a global shock, Dum is derived from the global ^SOX index). Time
...
the mitigating effect of AI exposure on the GPR shock's impact, net of
the GPR and F main effects (which are now separately estimated, not
folded into an implicit baseline). For the Corr spec specifically, the
mitigating effect of GPR*F is b4 alone in the bust regime (Dum=0) and
...
All results (IRF tables, all series used -- including both the raw GPR
...
Network access required (Eurostat API, matteoiacoviello.com,
...
SCRIPT_VERSION = "2025-08-11-v45-GPR-time-trend-control"  # bump this whenever the file changes;
...
# ----------------------------------------------------------------------
# 1. GPR (global) -- Caldara & Iacoviello
# ----------------------------------------------------------------------

def fetch_gpr_global():
    """
    Monthly global GPR index, Caldara & Iacoviello.
    Source page: https://www.matteoiacoviello.com/gpr.htm
    """
    ...
    q = df.resample("QE").mean()   # quarterly average of monthly index
    q.index = q.index.to_period("Q")
    return q


AR_PURIFICATION_LAGS = 2  # number of GPR own-lags used to purify the shock


def purify_gpr_shock(gpr_global_df, lags=AR_PURIFICATION_LAGS):
    """
    AR(lags) purification of the GPR index: regress GPR_t on its own lags
    and take the residual as an approximate "identified shock" -- the
    part of GPR not predictable from its own recent history.

    This is a much simpler stand-in for Caldara & Iacoviello's actual
    identification (an 8-variable structural VAR -- GPR, VIX, investment,
    hours, S&P 500, oil, 2y yield, NFCI -- with GPR ordered first in a
    Cholesky decomposition; see AER 2022, Section III). Their identified
    shock series is not published as a standalone downloadable file (only
    the GPR index itself is); reproducing it exactly would require
    re-estimating their full VAR. An AR-residual is the standard
    lightweight alternative used when a full structural VAR isn't
    warranted -- it strips out the persistent/predictable component of
    GPR, leaving something closer to a "surprise," but does NOT control
    for other macro/financial variables the way their VAR does. Treat
    this as a supplementary robustness check on the baseline spec, not a
    replacement for it or for the literature's own identified shock.

    Returns (df, model): df has columns quarter, gpr_global, gpr_shock;
    ...
    the residual -- a very low R^2 means GPR has little own-persistence
    to purify out in the first place, in which case gpr_shock will look
    a lot like gpr_global anyway).
    """
    import statsmodels.api as sm

    df = gpr_global_df[["quarter", "gpr_global"]].copy()
    df = df.sort_values("quarter").reset_index(drop=True)

    lag_cols = []
    for l in range(1, lags + 1):
        col = f"gpr_lag{l}"
        df[col] = df["gpr_global"].shift(l)
        lag_cols.append(col)
    ...
    df["gpr_shock"] = model.resid.values

    print(f"  [AR({lags}) GPR purification] R^2 = {model.rsquared:.3f}")
    print(f"  {model.params.to_string()}")

    return df[["quarter", "gpr_global", "gpr_shock"]], model


def compute_gpr_shock_std():
    """
    Fetches and AR-purifies the global GPR series (same steps as
    build_panel() does internally) and returns the standard deviation of
    the resulting gpr_shock -- the "1 standard deviation shock" size
    used to rescale the IRF charts from "response per 1 raw unit of
    gpr_used" (an arbitrary scale -- gpr_used is an AR(2) residual, not a
    naturally interpretable unit) into "response per 1-stdev GPR shock"
    (the standard, directly interpretable convention in the shock-IRF
    literature).

    Cheap and idempotent to call again here independently of
    build_panel() -- just a monthly-index download and a simple AR(2)
    regression, not a network-heavy operation, and calling it twice
    (once per exposure spec inside build_panel(), once here) always
    returns the same value since the underlying GPR series and AR
    specification don't change between calls.
    """
    gpr_global = fetch_gpr_global().reset_index().rename(columns={"month": "quarter"})
    shock_df, _ar_model = purify_gpr_shock(gpr_global)
    std = shock_df["gpr_shock"].std()
    print(f"  [diagnostic] GPR shock (gpr_used) standard deviation = {std:.4f} "
...
# uses the AR-purified shock as gpr_used, the baseline spec can reuse
# panel_investment/panel_corr directly (they already carry gpr_used = gpr_shock),
...
def fetch_gdp_growth_yoy():
    ...
    run_local_projections(), and run_baseline_gpr_projections()).
    ...
    run_local_projections()/run_baseline_gpr_projections()) is already a
...
def compute_sox_regime(window=SOX_REGIME_WINDOW_QUARTERS):
    ...
    cycle should DAMPEN the GPR shock's impact (Channel 1, tailwind
...
def build_panel(exposure="investment"):
    ...
    GPR is always the global Caldara-Iacoviello Geopolitical Risk Index,
    AR-purified (see purify_gpr_shock()): gpr_used is the
    AR(2) residual of the GPR level, not the raw level itself. This
    replaces the raw-level version used in earlier iterations of this
    script throughout -- the exposure interaction (gpr_x_exposure = F *
    gpr_used) and every downstream coefficient/IRF/chart are now based on
    the purified shock.
    """
    gpr_global = fetch_gpr_global().reset_index().rename(columns={"month": "quarter"})
    gpr_shock_df, _ar_model = purify_gpr_shock(gpr_global)
    ...
    panel = panel.merge(gpr_shock_df, on="quarter", how="left")
    panel["gpr_used"] = panel["gpr_shock"]  # AR-purified shock, not the raw level
    ...
    # global AI/ICT-adoption trend -- which risks the GPR*F interaction
    ...
    # own sensitivity -- that would require GPR*trend as a further
    ...
    # like gpr_used and is_boom -- estimable here only because this
    # script uses entity-only FE (no time FE, which would absorb it
    # completely, same reasoning as for the shock itself). It is NOT
    # perfectly collinear with gpr_used by construction (the AR(2)
    # purification in purify_gpr_shock() already strips out most smooth,
    ...
    # that b1 (gpr_used's own coefficient) and the time_trend
    ...
    _trend_shock_corr = panel[["time_trend", "gpr_used"]].dropna().corr().iloc[0, 1]
    print(f"  [diagnostic] corr(time_trend, gpr_used) = {_trend_shock_corr:.3f} "
    ...
    #
    # Investment spec:  GDPgrowth = b0 + b1*GPR + b2*F + b3*(GPR*F) + controls
    # Corr spec: GDPgrowth = b0 + b1*GPR + b2*F + b3*Dum + b4*(GPR*F)
    #                        + b5*(GPR*Dum) + b6*(GPR*F*Dum) + controls
    #            (Dum = is_boom; the reference level Dum=0 is "bust", so
    #            the mitigating effect of GPR*F is b4 in bust and b4+b6
    ...
    #
    # GPR (gpr_used) and F are included as separate regressors (b1, b2),
    ...
    #
    # IDENTIFICATION: gpr_used, is_boom, and their product (gpr_x_dum)
    # are entity-invariant (identical across every country at a given
    # quarter -- GPR is a global shock, is_boom is derived from the
    ...
    # would.
    # ------------------------------------------------------------------
    panel["gpr_x_exposure"] = panel["F"] * panel["gpr_used"]
    ...
    panel["gpr_used_focus"] = panel["is_focus"] * panel["gpr_used"]
    panel["F_focus"] = panel["is_focus"] * panel["F"]
    panel["gpr_x_exposure_focus"] = panel["is_focus"] * panel["gpr_x_exposure"]
    ...
    if exposure == "corr":
        ...
        panel["gpr_x_dum"] = panel["gpr_used"] * panel["is_boom"]                    # b5: GPR x Dum
        panel["gpr_x_exposure_x_dum"] = panel["gpr_x_exposure"] * panel["is_boom"]   # b6: GPR x F x Dum

        panel["is_boom_focus"] = panel["is_focus"] * panel["is_boom"]
        panel["gpr_x_dum_focus"] = panel["is_focus"] * panel["gpr_x_dum"]
        panel["gpr_x_exposure_x_dum_focus"] = panel["is_focus"] * panel["gpr_x_exposure_x_dum"]
...
def run_local_projections(panel, include_focus_interaction=True):
    ...
    - Investment spec:  gpr_used (b1), F (b2), gpr_x_exposure (b3, = GPR*F).
    - Corr spec: gpr_used (b1), F (b2), is_boom (b3, Dum), gpr_x_exposure
      (b4, GPR*F), gpr_x_dum (b5, GPR*Dum), gpr_x_exposure_x_dum (b6,
      GPR*F*Dum). The mitigating effect of GPR*F is b4 in the reference
    ...
    gpr_used, is_boom, and gpr_x_dum are entity-invariant (identical
    ...
    matters specifically because gpr_used/is_boom/gpr_x_dum are
    ...
    if has_boom_bust:
        base_terms = ["gpr_used", "F", "is_boom", "gpr_x_exposure", "gpr_x_dum",
                      "gpr_x_exposure_x_dum"]
        focus_terms = ["gpr_used_focus", "F_focus", "is_boom_focus", "gpr_x_exposure_focus",
                       "gpr_x_dum_focus", "gpr_x_exposure_x_dum_focus"]
    else:
        base_terms = ["gpr_used", "F", "gpr_x_exposure"]
        focus_terms = ["gpr_used_focus", "F_focus", "gpr_x_exposure_focus"]
    ...
    for h in HORIZONS:
        ...
        # run_baseline_gpr_projections() below.
...
def _linear_combination(res, weights: dict):
    ...
    sum of coefficients, e.g. {"gpr_x_exposure": 1, "gpr_x_exposure_x_dum": 1}
...
def summarize_irf(results, focus_country=FOCUS_COUNTRY):
    ...
    Investment spec:  b1_gpr, b2_F, b3_gpr_x_F
    Corr spec: b1_gpr, b2_F, b3_dum, b4_gpr_x_F, b5_gpr_x_dum,
               b6_gpr_x_F_x_dum
    ...
    if has_boom_bust:
        term_label_triples = [
            ("gpr_used", "gpr_used_focus", "b1_gpr"),
            ("F", "F_focus", "b2_F"),
            ("is_boom", "is_boom_focus", "b3_dum"),
            ("gpr_x_exposure", "gpr_x_exposure_focus", "b4_gpr_x_F"),
            ("gpr_x_dum", "gpr_x_dum_focus", "b5_gpr_x_dum"),
            ("gpr_x_exposure_x_dum", "gpr_x_exposure_x_dum_focus", "b6_gpr_x_F_x_dum"),
        ]
    else:
        term_label_triples = [
            ("gpr_used", "gpr_used_focus", "b1_gpr"),
            ("F", "F_focus", "b2_F"),
            ("gpr_x_exposure", "gpr_x_exposure_focus", "b3_gpr_x_F"),
        ]
    ...
    if has_boom_bust:
        for h, res in results.items():
            ...
            # alone -- already computed above under the b4_gpr_x_F label.
            row["mitigating_effect_bust_panelavg"] = row.get("beta_b4_gpr_x_F_panelavg", np.nan)
            row["se_mitigating_effect_bust_panelavg"] = row.get("se_b4_gpr_x_F_panelavg", np.nan)
            row[f"mitigating_effect_bust_{focus_country}_total"] = \
                row.get(f"beta_b4_gpr_x_F_{focus_country}_total", np.nan)
            row[f"se_mitigating_effect_bust_{focus_country}_total"] = \
                row.get(f"se_b4_gpr_x_F_{focus_country}_total", np.nan)

            # Mitigating effect in BOOM (Dum=1) = b4 + b6.
            b_boom, se_boom = _linear_combination(
                res, {"gpr_x_exposure": 1, "gpr_x_exposure_x_dum": 1}
            )
            ...
            b_boom_nl, se_boom_nl = _linear_combination(res, {
                "gpr_x_exposure": 1, "gpr_x_exposure_x_dum": 1,
                "gpr_x_exposure_focus": 1, "gpr_x_exposure_x_dum_focus": 1,
            })
...
# ----------------------------------------------------------------------
# 5b. Baseline GPR shock IRF -- NO exposure interaction, NO time effects
# ----------------------------------------------------------------------

def run_baseline_gpr_projections(panel):
    """
    The "raw" GDP response to a GPR shock, with the exposure/mitigation
    ...
    gpr_used = GPR_t is identical across every country at each t (a
    ...
    absent here by construction, since removing them would remove GPR's
    ...
    regressors = ["gpr_used", "dgdp_lag1", "time_trend"]
    ...
    for h in HORIZONS:
        ...
        res = mod.fit(cov_type="kernel", kernel="bartlett", bandwidth=bw)
        results[h] = res
        if "gpr_used" not in res.params.index:
            print(f"  [!] h={h}: gpr_used was ABSORBED and dropped -- "
                  f"not estimated at this horizon.")
...
def summarize_baseline_irf(results):
    """
    beta_gpr_baseline / se_gpr_baseline: the average GDP response to a
    one-unit GPR increase at horizon h, with exposure/mitigation switched
    off (see run_baseline_gpr_projections). Compare its t-stat
    ...
    for h, res in results.items():
        row = {"h": h}
        if "gpr_used" in res.params.index:
            b = res.params["gpr_used"]
            se = res.std_errors["gpr_used"]
            row["beta_gpr_baseline"] = b
            row["se_gpr_baseline"] = se
            row["t_stat_baseline"] = b / se if se > 0 else np.nan
            row["is_absorbed_baseline"] = False
        else:
            row["beta_gpr_baseline"] = np.nan
            row["se_gpr_baseline"] = np.nan
...
def plot_baseline_irf(irf_df, title="Baseline GPR shock IRF (no exposure interaction)",
                       shock_std=1.0):
    """
    Plots the unconditional, CUMULATIVE GPR-shock IRF with a 90% CI band,
    ...
    means "not significant at that h"). beta_gpr_baseline is already a
    cumulative coefficient -- see run_baseline_gpr_projections().

    shock_std: rescales the plotted beta/se from "response per 1 RAW
    unit of gpr_used" (an arbitrary scale -- gpr_used is an AR(2)
    residual, not a naturally interpretable unit) into "response per
    1-STANDARD-DEVIATION GPR shock" -- pass compute_gpr_shock_std()'s
    ...
    h = irf_df["h"]
    b = irf_df["beta_gpr_baseline"] * shock_std
    se = irf_df["se_gpr_baseline"] * shock_std
    ax.plot(h, b, marker="o", color="#2ca02c", label="Baseline GPR effect")
    ax.fill_between(h, b - 1.645 * se, b + 1.645 * se, alpha=0.2, color="#2ca02c")
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Horizon h (quarters)")
    ax.set_ylabel("Cumulative dGDP response per 1-stdev GPR shock (0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
def plot_irf(irf_df, title, focus_country=FOCUS_COUNTRY, shock_std=1.0):
    """
    Plots the panel-average and NL-specific CUMULATIVE b3 (GPR*F
    ...
    net of the GPR and F main effects (see run_local_projections()).
    Returns the matplotlib Figure (caller decides whether to
    save/embed/show it).

    shock_std: rescales the plotted beta/se from "per 1 raw unit of
    gpr_used" into "per 1-stdev GPR shock" -- see plot_baseline_irf()'s
    ...
    h = irf_df["h"]
    b_panel = irf_df["beta_b3_gpr_x_F_panelavg"] * shock_std
    se_panel = irf_df["se_b3_gpr_x_F_panelavg"] * shock_std
    ...
    col_nl = f"beta_b3_gpr_x_F_{focus_country}_total"
    se_nl_col = f"se_b3_gpr_x_F_{focus_country}_total"
    ...
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Horizon h (quarters)")
    ax.set_ylabel("Cumulative GPR x F interaction effect per 1-stdev GPR shock (b3, 0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
def plot_combined_mitigating_irf(irf_patent, irf_investment, shock_std=1.0,
    ...
    the GPR*F interaction) from TWO separately-estimated Spec A models
    ...
    "does the GPR shock's growth impact respond to AI-patenting
    ...
    GPR-shock construction and the SAME HORIZONS grid, so their "h"
    ...
    b_pat = irf_patent["beta_b3_gpr_x_F_panelavg"] * shock_std
    se_pat = irf_patent["se_b3_gpr_x_F_panelavg"] * shock_std
    ...
    b_inv = irf_investment["beta_b3_gpr_x_F_panelavg"] * shock_std
    se_inv = irf_investment["se_b3_gpr_x_F_panelavg"] * shock_std
...
def plot_regime_irf(irf_df, regime, title, focus_country=FOCUS_COUNTRY, color="#2ca02c",
    ...
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Horizon h (quarters)")
    ax.set_ylabel("Cumulative mitigating effect per 1-stdev GPR shock (0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
VARIABLE_DEFINITIONS = {
    ...
    "dgdp_lag1": "dgdp (year-on-year GDP growth), lagged 1 quarter -- one of two controls used throughout this script (Investment spec, Corr spec, and the baseline spec alike), alongside time_trend; a second GDP lag (dgdp_lag2) is not used.",
    "time_trend": "Explicit LINEAR TIME TREND control: an integer counter (0, 1, 2, ...) over the panel's own quarters, entity-invariant (same value for every country at a given quarter). Added because F trends upward over most of the sample for nearly every country at once (a shared AI/ICT-adoption trend), which risks the GPR*F interaction coefficient partly reflecting a generic drift in growth over calendar time rather than something specifically tied to exposure. Does NOT fully resolve this (a plain additive trend controls for drift in the LEVEL of growth, not specifically for drift in the shock's own sensitivity -- that would need a further GPR*time_trend interaction, not included here). See build_panel() for the printed corr(time_trend, gpr_used) diagnostic and its interpretation.",
    "gpr_global": "Global Geopolitical Risk Index (RAW LEVEL), monthly, quarterly-averaged (Caldara & Iacoviello, AER 2022) -- kept in the panel for reference, but NOT what the model estimates on (see gpr_used)",
    "gpr_used": "The GPR series actually used in the model: gpr_shock, the AR(2)-purified GPR shock (see purify_gpr_shock()) -- NOT the raw level. Every coefficient, IRF, and chart in this workbook is based on this purified shock.",
    ...
    "gpr_x_exposure": "b3 (Investment spec) / b4 (Corr spec) interaction regressor: F * gpr_used. Its coefficient is the mitigating effect NET OF the GPR and F main effects (gpr_used, F), which are now separately included in the regression -- not a 'difference' spec.",
    "is_focus": f"1 if country == FOCUS_COUNTRY ({FOCUS_COUNTRY}), 0 otherwise",
    "gpr_used_focus": "is_focus * gpr_used -- NL's own b1 (GPR main effect) interaction term",
    "F_focus": "is_focus * F -- NL's own b2 (F main effect) interaction term",
    "gpr_x_exposure_focus": "is_focus * gpr_x_exposure -- NL's own interaction-term deviation (b3 for Investment spec, b4 for Corr spec)",
    ...
    "gpr_x_dum": "b5 (Corr spec): gpr_used * is_boom (GPR x Dum interaction). Entity-invariant like gpr_used and is_boom themselves -- estimable only because this spec uses entity-only fixed effects (see run_local_projections()).",
    "gpr_x_dum_focus": "is_focus * gpr_x_dum -- NL's own b5 interaction term. Corr spec only.",
    "gpr_x_exposure_x_dum": "b6 (Corr spec): gpr_x_exposure * is_boom (GPR x F x Dum, the full three-way interaction). Corr spec only.",
    "gpr_x_exposure_x_dum_focus": "is_focus * gpr_x_exposure_x_dum -- NL's own b6 interaction term. Corr spec only.",
    "h": "Local-projection horizon, in quarters",
    "beta_b1_gpr_panelavg": "Panel-average CUMULATIVE coefficient on gpr_used (b1, the GPR main effect) over horizons 0..h. Both Investment and Corr specs.",
    "se_b1_gpr_panelavg": "Standard error of beta_b1_gpr_panelavg (Driscoll-Kraay-style)",
    ...
    "beta_b3_gpr_x_F_panelavg": "Panel-average CUMULATIVE coefficient on gpr_x_exposure (b3, Investment spec's GPR*F interaction) over horizons 0..h -- the Investment spec's mitigating-effect coefficient, net of the GPR and F main effects. A POSITIVE value means exposure dampens/offsets the (typically negative) GPR shock response; negative means amplification.",
    "se_b3_gpr_x_F_panelavg": "Standard error of beta_b3_gpr_x_F_panelavg",
    f"beta_b1_gpr_{FOCUS_COUNTRY}_total": f"{FOCUS_COUNTRY}'s total b1 (GPR main effect) coefficient (panel-average + deviation)",
    f"se_b1_gpr_{FOCUS_COUNTRY}_total": f"Standard error of {FOCUS_COUNTRY}'s total b1 coefficient",
    ...
    f"beta_b3_gpr_x_F_{FOCUS_COUNTRY}_total": f"{FOCUS_COUNTRY}'s total b3 (Investment spec's GPR*F interaction) coefficient (panel-average + deviation) -- {FOCUS_COUNTRY}'s own mitigating-effect coefficient",
    f"se_b3_gpr_x_F_{FOCUS_COUNTRY}_total": f"Standard error of {FOCUS_COUNTRY}'s total b3 coefficient",
    ...
    "beta_b4_gpr_x_F_panelavg": "Panel-average CUMULATIVE coefficient on gpr_x_exposure (b4, Corr spec's GPR*F interaction) over horizons 0..h -- this is ALSO the mitigating effect in the reference regime (Dum=0, 'bust'); see mitigating_effect_bust_panelavg, which equals this exactly.",
    "se_b4_gpr_x_F_panelavg": "Standard error of beta_b4_gpr_x_F_panelavg",
    "beta_b5_gpr_x_dum_panelavg": "Panel-average CUMULATIVE coefficient on gpr_x_dum (b5, GPR*Dum interaction) over horizons 0..h. Corr spec only.",
    "se_b5_gpr_x_dum_panelavg": "Standard error of beta_b5_gpr_x_dum_panelavg",
    "beta_b6_gpr_x_F_x_dum_panelavg": "Panel-average CUMULATIVE coefficient on gpr_x_exposure_x_dum (b6, the full three-way GPR*F*Dum interaction) over horizons 0..h -- this is the ADDITIONAL mitigating effect specific to the boom regime, on top of b4. Corr spec only.",
    "se_b6_gpr_x_F_x_dum_panelavg": "Standard error of beta_b6_gpr_x_F_x_dum_panelavg",
    "mitigating_effect_bust_panelavg": "The mitigating effect of GPR*F in the BUST regime (Dum=0, the reference level) = b4 alone. Expected NEGATIVE if shared/concentrated exposure to the AI-chip cycle amplifies the (typically negative) GPR shock response during downturns.",
    "se_mitigating_effect_bust_panelavg": "Standard error of mitigating_effect_bust_panelavg (= se_b4_gpr_x_F_panelavg exactly, since the bust effect IS b4)",
    "mitigating_effect_boom_panelavg": "The mitigating effect of GPR*F in the BOOM regime (Dum=1) = b4 + b6, with a correctly derived COMBINED standard error (via the full coefficient covariance matrix, NOT b4's and b6's individual SEs summed naively). Expected POSITIVE if the demand/terms-of-trade tailwind dampens the GPR shock response during AI-capex booms.",
    ...
    "beta_gpr_baseline": "Average CUMULATIVE GDP response to a one-unit GPR shock increase over horizons 0..h, WITHOUT the exposure/mitigation interaction (entity FE only, no time FE -- see run_baseline_gpr_projections)",
    "se_gpr_baseline": "Standard error of beta_gpr_baseline",
    "t_stat_baseline": "t-statistic of beta_gpr_baseline (beta/se) -- |t|>~1.96 is significant at the 5% level",
    "is_absorbed_baseline": "True if gpr_used was fully absorbed at this horizon",
    "gpr_shock": "Residual from an AR(2) regression of gpr_global on its own lags -- an approximate 'identified shock' (the unpredictable part of GPR). This IS gpr_used throughout the model now. See purify_gpr_shock() for why this is a simplified stand-in for a fully identified structural shock, not a reproduction of one.",
...
if __name__ == "__main__":
    print("=" * 70)
    print("Computing GPR shock standard deviation (for 1-stdev-shock IRF charts)")
    print("=" * 70)
    gpr_shock_std = compute_gpr_shock_std()

    print("\n" + "=" * 70)
    print("SPEC A (i): AI patent applications as exposure, global GPR")
    ...
    print("\n" + "=" * 70)
    print("SPEC A (ii): AI incoming investment counts as exposure, global GPR")
    print("             (table only -- see the combined chart further below)")
    print("=" * 70)
    ...
    print("=" * 70)
    fig_combined = plot_combined_mitigating_irf(irf_patent, irf_investment, shock_std=gpr_shock_std)

    print("\n" + "=" * 70)
    print("SPEC B: Rolling correlation(national index, semiconductor index)")
    print("        as exposure (robustness), global GPR")
    ...
    print("\n" + "=" * 70)
    print("BASELINE: GPR shock IRF, exposure/mitigation switched off")
    print("          (entity FE only, no time FE -- see docstring for why;")
    print("          gpr_used = AR-purified shock, not the raw GPR level)")
    print("          (table only -- not charted, per this script's design)")
    print("=" * 70)
    # gpr_used and dgdp are identical across specs (only the exposure proxy
    # differs), so this only needs to be run once, off any panel.
    res_baseline = run_baseline_gpr_projections(panel_patent)
    irf_baseline = summarize_baseline_irf(res_baseline)
    print(irf_baseline.to_string())

    print("\n" + "=" * 70)
    print("Fetching raw source series for export (GDP, AI patents, AI investment, indices, GPR)")
    print("=" * 70)
    ...
    raw_gpr_level = fetch_gpr_global().reset_index().rename(columns={"month": "quarter"})
    raw_gpr_shock, _ar_model = purify_gpr_shock(raw_gpr_level)  # quarter, gpr_global, gpr_shock
    ...
    export_results_to_excel(
        ...
        raw_data={
            ...
            "Raw_GPR_Level": raw_gpr_level,
            "Raw_GPR_Shock": raw_gpr_shock,  # quarter, gpr_global, gpr_shock -- what the model actually uses
    ...
    Patent spec:      GDPgrowth = b0 + b1*GPR + b2*F + b3*(GPR*F) + controls
    Investment spec:  GDPgrowth = b0 + b1*GPR + b2*F + b3*(GPR*F) + controls
    ...
    ENTITY fixed effects only, no time effects -- required because GPR,
    ...
    the tables), look at beta_b3_gpr_x_F_{FOCUS_COUNTRY}_total and
    se_b3_gpr_x_F_{FOCUS_COUNTRY}_total in the Patent_exposure_IRF or
    ...
    exposure (to that channel specifically) dampens the GPR shock's
    impact on growth; NEGATIVE means exposure amplifies it. beta_b1_gpr_*
    and beta_b2_F_* report the GPR and F main effects themselves,
    ...
    dampens the GPR shock's impact during an AI-capex boom) and
    ...
    NOTE ON SPECIFICATION: GPR_shock is the AR(2)-purified shock (see
    purify_gpr_shock()), not the raw GPR level -- this applies throughout
    the workbook, including the baseline spec below.

    BASELINE GPR IRF (Baseline_GPR sheet): the unconditional GDP response
    to the AR-purified GPR shock, with the exposure/mitigation channel
    ...
    FE -- so read it as "does the GPR shock move growth on average
    across the panel", not as a fully "clean" two-way-FE IRF. The
    Raw_GPR_Level and Raw_GPR_Shock sheets let you compare the purified
    ...
    5. ALL FOUR specs (Patent, Investment, Corr, Baseline) use entity
       ...
       GPR/Dum main effects -- see the model specification note above).
    ...
    7. THE ONLY CHART in this workbook (Combined_Patent_vs_Investment, in
       ...
       to show the cumulative dGDP response per 1-STANDARD-DEVIATION GPR
       shock (gpr_shock_std, printed above -- see compute_gpr_shock_std())
       rather than per 1 raw unit of gpr_used (an AR(2) residual, an
       ...
       Corr_exposure_IRF, Baseline_GPR_IRF) for anyone who wants to plot
       ...
       needs to multiply by gpr_shock_std manually to match what the
    ...
    The baseline spec asks a simple question: does a GPR (uncertainty)
    shock move GDP growth at all, on average across the panel? beta_gpr_
    ...
    same shock? This is captured by an INTERACTION term (GPR shock x
    ...
    marginal GDP response to a GPR shock, as a function of exposure F, is

        d(GDP growth) / d(GPR) = b1 + b3*F        (Patent / Investment spec)
        d(GDP growth) / d(GPR) = b1 + b4 + b6*Dum  (Corr spec, at F=1)
