...
semiconductor, and MSCI World equity indices, all five global shock
series (WUI, GPR, EPU, TPU, GSCPI), and the AI/ICT-related HS export
data -- and writes them all into a single local workbook, ai_data.xlsx,
with exactly the sheet structure the rest of this project's scripts
(the local-data modeling scripts) expect: gdp, ict_inv, ai_patent,
ai_inv, index_nat, index_sox, index_msci, hs_export, wui, gpr, epu, tpu,
gscpi, com, trade_openness, population_growth, productivity_growth,
gdp_per_capita, infl.
...
def fetch_ai_investment(gdp_df):
    ...
    country-year's AVERAGE quarterly GDP level (from gdp_df, the same
    DataFrame fetch_gdp_level() returns), times 1000 -- following the
    exact formula already present in cell D2 of the attached reference
    workbook's ai_inv sheet: "=(C2/AVERAGE(gdp!C62:C65))*1000", where
    C62:C65 are that row's own country's 4 quarters of the matching
    year in the "gdp" sheet. This function computes the same VALUE in
    Python (used as the cached result behind a live formula written in
    main() below, which is what actually reproduces "C62:C65" as a
    real cross-sheet Excel reference rather than a fixed number).
    ...
    # --- "share" column: ai_investment / that country-year's average
    # quarterly GDP level, x1000 -- matching the reference workbook's
    # D2 formula exactly (see this function's docstring). gdp_df's
    # "quarter" column is a string like "2015Q1"; matching on its first
    # 4 characters against the target year, per country, mirrors how
    # the reference formula's own C62:C65 range was built (that
    # specific country's 4 quarters of that specific year).
    gdp = gdp_df.copy()
    gdp["gdp_year"] = gdp["quarter"].astype(str).str[:4].astype(int)
    gdp_annual_avg = (gdp.groupby(["country", "gdp_year"])["gdp_level"]
                      .mean().rename("avg_gdp_level").reset_index())

    long_filtered = long_filtered.merge(
        gdp_annual_avg, left_on=["country", "year"], right_on=["country", "gdp_year"],
        how="left",
    )
    missing_gdp = long_filtered["avg_gdp_level"].isna().sum()
    if missing_gdp:
        print(f"  [!] fetch_ai_investment: {missing_gdp} country-year rows have no "
              f"matching GDP data (share will be blank for these) -- check gdp_df's "
    ...
    long_filtered["share"] = (long_filtered["ai_investment"]
                               / long_filtered["avg_gdp_level"]) * 1000
...
def fetch_msci_world_raw():
    ...
    sandbox (no network access here). The diagnostic warning below
    fires loudly and specifically if this assumption turns out wrong
    on a real run, so a stale/incorrect ticker cannot fail silently.
    """
    ticker = "^990100-USD-STRD"
    px = _yf_close_series(ticker)
    if px.empty:
        print(f"  [!] WARNING: yfinance returned ZERO rows for MSCI World "
              ...
              f"Finance. Try searching \"MSCI World\" on "
    ...
    print(f"  [diagnostic] MSCI World index ({ticker}): {len(result)} rows, "
          f"earliest quarter: {earliest}.")
    if earliest is not None and str(earliest) > "2000Q2":
        print(f"  [!] WARNING: earliest available quarter is {earliest}, "
              ...
              f"2000. Check https://finance.yahoo.com/quote/%5E990100-USD-STRD/"
...
if __name__ == "__main__":
    ...
    print("\n[1/19] GDP level (Eurostat)...")
    sheets["gdp"] = fetch_gdp_level()

    print("\n[2/19] ICT investment share (Eurostat)...")
    sheets["ict_inv"] = fetch_ict_investment_share()

    print("\n[3/19] AI patent applications (ETO/CSET, local file)...")
    sheets["ai_patent"] = fetch_ai_patents()

    print("\n[4/19] AI incoming investment counts (ETO/CSET, local file)...")
    sheets["ai_inv"] = fetch_ai_investment(sheets["gdp"])

    print("\n[5/19] National equity indices (Yahoo Finance)...")
    sheets["index_nat"] = fetch_national_indices_raw()

    print("\n[6/19] Semiconductor index ^SOX (Yahoo Finance)...")
    sheets["index_sox"] = fetch_semiconductor_raw()

    print("\n[7/19] MSCI World index (Yahoo Finance)...")
    sheets["index_msci"] = fetch_msci_world_raw()

    print("\n[8/19] AI/ICT-related HS export data (OECD BIMTS)...")
    sheets["hs_export"] = fetch_hs_export_data()

    print("\n[9/19] World Uncertainty Index (WUI)...")
    sheets["wui"] = fetch_wui_global()

    print("\n[10/19] Geopolitical Risk Index (GPR)...")
    sheets["gpr"] = fetch_gpr_global()

    print("\n[11/19] Economic Policy Uncertainty Index (EPU)...")
    sheets["epu"] = fetch_epu_global()

    print("\n[12/19] Trade Policy Uncertainty Index (TPU)...")
    sheets["tpu"] = fetch_tpu_global()

    print("\n[13/19] Global Supply Chain Pressure Index (GSCPI)...")
    sheets["gscpi"] = fetch_gscpi_global()

    print("\n[14/19] Global commodity price index, incl. oil and gas (FRED)...")
    sheets["com"] = fetch_commodity_price_index()

    print("\n[15/19] Trade openness (World Bank)...")
    sheets["trade_openness"] = fetch_trade_openness()

    print("\n[16/19] Population growth (World Bank)...")
    sheets["population_growth"] = fetch_population_growth()

    print("\n[17/19] Labor productivity growth: real productivity per hour worked, YoY (Eurostat)...")
    sheets["productivity_growth"] = fetch_productivity_growth()

    print("\n[18/19] GDP per capita (Eurostat)...")
    sheets["gdp_per_capita"] = fetch_gdp_per_capita()

    print("\n[19/19] HICP index level, 2015=100 (Eurostat)...")
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine="xlsxwriter") as writer:
        for sheet_name, df in sheets.items():
            ...
            if sheet_name == "ict_inv":
                ...
                # Formula matches the reference workbook's own D2 example
                # exactly: "=(C2/AVERAGE(gdp!C62:C65))*1000" -- C{row} is
                # this row's own ai_investment, and the gdp!C{..}:C{..}
                # range is dynamically located below (not hardcoded) to
                # that SAME country's 4 quarterly rows for the matching
                # year in the "gdp" sheet, found via the in-memory gdp
                ...
                gdp_df = sheets["gdp"]
                gdp_years = gdp_df["quarter"].astype(str).str[:4]
                for i, row in df.iterrows():
                    excel_row = i + 2
                    match_positions = gdp_df.index[
                        (gdp_df["country"] == row["country"])
                        & (gdp_years == str(int(row["year"])))
                    ].tolist()
                    if not match_positions:
                        # No matching GDP quarters for this country-year --
                        # write a plain 0 (not a formula referencing an
                        # empty/invalid range), matching fetch_ai_investment()'s
                        # own diagnostic warning for this same condition.
                        worksheet.write(i + 1, col_d, 0.0)
                        continue
                    gdp_start_row = match_positions[0] + 2
                    gdp_end_row = match_positions[-1] + 2
                    formula = (f"=(C{excel_row}/AVERAGE(gdp!C{gdp_start_row}:"
                               f"C{gdp_end_row}))*1000")
                    share_val = row["share"]
