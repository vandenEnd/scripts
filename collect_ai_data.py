...
index_stoxx600, hs_export, wui, gpr, epu, tpu, gscpi, com,
...
if __name__ == "__main__":
    ...
    print("\n[1/20] GDP level (Eurostat)...")
    sheets["gdp"] = fetch_gdp_level()

    print("\n[2/20] ICT investment share (Eurostat)...")
    sheets["ict_inv"] = fetch_ict_investment_share()

    print("\n[3/20] AI patent applications (ETO/CSET, local file)...")
    sheets["ai_patent"] = fetch_ai_patents()

    print("\n[4/20] AI incoming investment counts (ETO/CSET, local file)...")
    sheets["ai_inv"] = fetch_ai_investment(sheets["gdp"])

    print("\n[5/20] National equity indices (Yahoo Finance)...")
    sheets["index_nat"] = fetch_national_indices_raw()

    print("\n[6/20] Semiconductor index ^SOX (Yahoo Finance)...")
    sheets["index_sox"] = fetch_semiconductor_raw()

    print("\n[7/20] MSCI World index (Yahoo Finance)...")
    sheets["index_msci"] = fetch_msci_world_raw()

    print("\n[8/20] STOXX Europe 600 index (Yahoo Finance)...")
    sheets["index_stoxx600"] = fetch_stoxx600_raw()

    print("\n[9/20] AI/ICT-related HS export data (OECD BIMTS)...")
    sheets["hs_export"] = fetch_hs_export_data()

    print("\n[10/20] World Uncertainty Index (WUI)...")
    sheets["wui"] = fetch_wui_global()

    print("\n[11/20] Geopolitical Risk Index (GPR)...")
    sheets["gpr"] = fetch_gpr_global()

    print("\n[12/20] Economic Policy Uncertainty Index (EPU)...")
    sheets["epu"] = fetch_epu_global()

    print("\n[13/20] Trade Policy Uncertainty Index (TPU)...")
    sheets["tpu"] = fetch_tpu_global()

    print("\n[14/20] Global Supply Chain Pressure Index (GSCPI)...")
    sheets["gscpi"] = fetch_gscpi_global()

    print("\n[15/20] Global commodity price index, incl. oil and gas (FRED)...")
    sheets["com"] = fetch_commodity_price_index()

    print("\n[16/20] Trade openness (World Bank)...")
    sheets["trade_openness"] = fetch_trade_openness()

    print("\n[17/20] Population growth (World Bank)...")
    sheets["population_growth"] = fetch_population_growth()

    print("\n[18/20] Labor productivity growth: real productivity per hour worked, YoY (Eurostat)...")
    sheets["productivity_growth"] = fetch_productivity_growth()

    print("\n[19/20] GDP per capita (Eurostat)...")
    sheets["gdp_per_capita"] = fetch_gdp_per_capita()

    print("\n[20/20] HICP index level, 2015=100 (Eurostat)...")
