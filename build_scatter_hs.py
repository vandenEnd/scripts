...
def checked_sheet_name(name, context=""):
    """
    Return `name` unchanged, but FAIL LOUDLY if it exceeds Excel's
    31-character sheet-name limit.

    openpyxl SILENTLY TRUNCATES an over-long sheet name -- it only
    emits a UserWarning that is trivial to miss in a long run, and the
    workbook then contains a sheet whose real name is one or more
    characters shorter than the name this code thinks it created. Any
    later lookup or cross-reference by that name then fails, or worse,
    two distinct names collapse onto the same truncated name and one
    sheet silently overwrites the other. Raising here turns that
    silent corruption into an immediate, explicit error naming the
    offending key, so renaming a variable key can never quietly break
    sheet naming.
    """
    if len(name) > 31:
        raise SystemExit(
            f"\nSheet name '{name}' is {len(name)} characters, over Excel's "
            f"31-character limit, and would be SILENTLY TRUNCATED by "
            f"openpyxl{(' (' + context + ')') if context else ''}. Shorten "
            f"the key used to build it."
        )
    return name
...
# supplies State aid expenditure at t; column D supplies nominal GDP, shifted
# one year forward so outcome/control year t uses GDP from t-1. This removes
# the contemporaneous-GDP denominator effect without lagging the aid numerator.
...
def align_previous_year_explanatory(df):
    ...
    After this transformation, merging on (country, target_year) makes every
    outcome in year t use that country's observed explanatory value in t-1.
    The source year is retained for transparent workbook diagnostics.
    """
    lagged = df.copy()
...
hs_export_lagged = align_previous_year_explanatory(hs_export)
...
merged_hs = forecast.merge(hs_export_lagged, on=["country", "target_year"], how="inner")
...
print(f"previous-year hs_export share merge: {len(merged_hs)} rows "
      f"(from {len(forecast)} forecast rows)")
print(f"previous-year stock/semis correlation merge: {len(merged_corr)} rows "
      f"(from {len(forecast)} forecast rows)")
print(f"previous-year ai_inv share merge: {len(merged_ai_inv)} rows "
      f"(from {len(forecast)} forecast rows)")
...
specs = [
    ...
    ("hs_export_share", merged_hs, "hs_export_share", "Previous-year AI/ICT-related HS export share", "D62728",
     "vs. Previous-year AI/ICT-related HS export share"),
    ("stock_corr", merged_corr, "stock_semis_corr_annual",
     ...
     "vs. previous-year national-semiconductor index correlation"),
...
for var_key, df, col, header, color, title_suffix in specs:
    ...
    for period_key, min_year, max_year, period_label in periods:
        ...
        data_sheet_name = checked_sheet_name(
            f"data_{var_key}_{period_key}", f"period sheets, var_key={var_key!r}")
        chart_sheet_name = checked_sheet_name(
            f"chart_{var_key}_{period_key}", f"period sheets, var_key={var_key!r}")
...
def country_clustered_share_test(sample1, sample2, symbol="*"):
    ...
    That matters for the shock-years versus all-years comparisons, because
    shock years are part of the all-years sample. Clustering the duplicated
    observations under the same country retains their dependence instead of
    incorrectly treating the two percentages as independent.
    """
...
def _share_observations(df):
    """Return country-level identifiers and a positive-surprise indicator."""
...
def write_clustered_time_chart(wb, cluster_key, merged_df, cluster_col, cluster_header,
    ...
    data_sheet_name = checked_sheet_name(
        f"data_cluster_{cluster_key}", f"cluster sheet, cluster_key={cluster_key!r}")
...
def write_special_years_chart(wb, var_key, raw_forecast, raw_explanatory, col, header, color,
    ...
    exactly t-1. There is no same-year lookup and no fallback to another
    year: if country i has no value in t-1, that outcome observation is
    excluded. The output's explanatory_year_used column makes the lag
    explicit for every retained row.
    """
    ...
    data_sheet_name = checked_sheet_name(
        f"data_{var_key}_yrs", f"special-years sheet, var_key={var_key!r}")
    chart_sheet_name = checked_sheet_name(
        f"chart_{var_key}_yrs", f"special-years sheet, var_key={var_key!r}")
...
header_row = summary_row + 1
summary_row, cluster_props = write_clustered_time_chart(
    wb, "hs_export", merged_hs, "hs_export_share", "Previous-year AI/ICT-related HS export share",
...
_, yrs_props, yrs_df_hs = write_special_years_chart(
    ...
    "Previous-year AI/ICT-related HS export share", "9467BD",
    ws_summary, header_row - 1, col_offset=4)
all_share_tests["Previous-year AI/ICT-related HS export share"] = add_significance_stars(
    ws_summary, header_row, cluster_props, yrs_props)
all_props["Previous-year AI/ICT-related HS export share"] = (cluster_props, yrs_props)
block_header_rows["Previous-year AI/ICT-related HS export share"] = header_row
...
j_col_specs = [
    ...
    "Previous-year AI/ICT-related HS export share",
...
def _state_aid_diagnostic_cols():
    """Raw inputs and timing fields shown in data_regr when control is on."""
    if not INCLUDE_STATE_AID_CONTROL:
        return []
    return [
...
regression_specs = [
    ...
    ("Previous-year AI/ICT-related HS export share (data_hs_export_share_full)", merged_hs, "hs_export_share"),
...
variable_chart_specs = [
    ...
    ("Previous-year AI/ICT-related HS export share", "ICT/AI  export", "ICTAI  export"),
...
# X replaced by the above-median-value transform of previous-year X:
# X(t-1) where that value exceeds the variable's overall median, and 0
# otherwise. It therefore directly
...
above_median_specs = [
    ...
    ("Previous-year AI/ICT-related HS export share, ABOVE MEDIAN value (data_hs_export_share_full)",
...
for offset, (label, df_src, x_col) in enumerate(above_median_specs):
    ...
    df_src["growth_surprise"] = (df_src["realized_growth_annual_pct"]
                                  - df_src["forecast_growth_annual_pct"])
# Median computed on the SAME (full-sample) previous-year X data this regression
    ...
    # script.
    #
    # CONTINUOUS version, per explicit instruction: "above_median_raw" here
...
table_display_names = [
    ...
    ("Previous-year AI/ICT-related HS export share", "ICT/AI-related export"),
...
note_lines = [
    ...
    "and phi, SE(phi), and p(phi) appear only in the detailed tables.",
