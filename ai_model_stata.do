...
* ("Data_AI.xlsx"), which must sit next to this do-file. No network access
* of any kind. Expected sheets: gdp, ict_inv, ai_patent, ai_inv, index_nat,
* index_sox, wui, gpr (see the import section below for exact columns).
*
...
*
* HONESTY NOTE: this do-file was translated from a fully working, tested
* Python/pandas/statsmodels/linearmodels pipeline. It has NOT been run in
* Stata (not available in the environment this was written in) -- treat
* this as a careful first draft, not a verified-correct final version. Run
* it, and if/where it errors, that is expected and fixable; report the
* exact error back for a fix, the same way the Python version was
...
*-----------------------------------------------------------------------
* 1. Import all sheets from the local workbook into separate .dta files
*    (Stata works with one dataset in memory at a time, unlike pandas --
*    each sheet is imported once here and saved to a temp .dta, then
...
*-----------------------------------------------------------------------
tempfile gdp_dta ict_inv_dta ai_patent_dta ai_inv_dta index_nat_dta ///
    index_sox_dta shock_dta
...
save "shock_innov_dta.dta", replace
...
save "gdp_growth_dta.dta", replace
...
use `index_nat_dta', clear
encode country, gen(country_id)
xtset country_id quarter
tempfile nat_ret_dta
save `nat_ret_dta'

use `index_sox_dta', clear
tsset quarter
rename log_ret sox_log_ret
keep quarter sox_log_ret close
rename close sox_close
tempfile sox_ret_dta
save `sox_ret_dta'
...
use `nat_ret_dta', clear
merge m:1 quarter using `sox_ret_dta', keep(match) nogenerate
...
clear
tempfile corr_exposure_dta
save `corr_exposure_dta', emptyok replace

foreach cid of local idlist {
    use `corr_input', clear
    ...
    append using `corr_exposure_dta'
    save `corr_exposure_dta', replace
}
use `corr_exposure_dta', clear
sort country_id quarter
save `corr_exposure_dta', replace
...
*    to the .dta file named in `2' (the second argument).
*-----------------------------------------------------------------------
...
display "DEBUG: entering build_panel"
capture confirm file "gdp_growth_dta.dta"
if _rc {
	di as error "gdp_growth_dta.dta not found"
	error 601
	}

use "gdp_growth_dta.dta", clear
    if "`exposure'" == "ict" {
        ...
        * value across all 4 quarters of that calendar year)
        ...
        merge m:1 country year using `ict_annual', keep(master match) nogenerate
        drop year
    }
    else if "`exposure'" == "patent" {
        preserve
        use `ai_patent_dta', clear
        rename ai_patents z_raw
        keep country year z_raw
        tempfile patent_annual
        save `patent_annual'
        restore
        gen year = yofd(dofq(quarter))
        merge m:1 country year using `patent_annual', keep(master match) nogenerate
        drop year
    }
    else if "`exposure'" == "investment" {
        preserve
        use `ai_inv_dta', clear
        rename ai_investment z_raw
        keep country year z_raw
        tempfile investment_annual
        save `investment_annual'
        restore
        gen year = yofd(dofq(quarter))
        merge m:1 country year using `investment_annual', keep(master match) nogenerate
        drop year
    }
        ...
        * panel, one for corr_exposure_dta, built in different sections)
        * happening to assign identical numeric codes; string keys can't
        * silently mismatch that way.
        merge 1:1 country quarter using `corr_exposure_dta', ///
    ...
    * merge in the AR-purified shock (entity-invariant, merges on quarter)
    merge m:1 quarter using "shock_innov_dta.dta"
    ...
    * NL-specific deviations from the panel-average coefficients.
    ...
    if "`exposure'" == "corr" {
        merge m:1 quarter using `sox_regime_dta', keepusing(is_boom) ///
            keep(master match) nogenerate
...
program define run_local_projections
    ...
    *       2 = output results .dta path (one row per h x coefficient)
    ...
    forvalues h = 0/$horizons_max {
        ...
        quietly xtscc dgdp_cum_lead `all_terms' dgdp_lag1, fe lag(`bw')
...
program define run_baseline_shock_projections
    ...
    forvalues h = 0/$horizons_max {
        ...
        quietly xtscc dgdp_cum_lead shock_used dgdp_lag1, fe lag(`bw')
...
* 9. RUN ALL SPECS
*-----------------------------------------------------------------------
tempfile panel_ict panel_patent panel_investment panel_corr
tempfile irf_ict irf_patent irf_investment irf_corr irf_baseline

di as result _n "{hline 70}"
di as result "SPEC A: ICT investment share as exposure, global $shock_variable"
di as result "{hline 70}"
build_panel ict "`panel_ict'"
run_local_projections "`panel_ict'" "`irf_ict'"

di as result _n "{hline 70}"
di as result "SPEC A (patent): AI patent applications as exposure, global $shock_variable"
di as result "{hline 70}"
build_panel patent "`panel_patent'"
run_local_projections "`panel_patent'" "`irf_patent'"

di as result _n "{hline 70}"
di as result "SPEC A (investment): AI incoming investment counts as exposure, global $shock_variable"
di as result "{hline 70}"
build_panel investment "`panel_investment'"
run_local_projections "`panel_investment'" "`irf_investment'"
...
di as result "{hline 70}"
build_panel corr "`panel_corr'"
run_local_projections "`panel_corr'" "`irf_corr'"

di as result _n "{hline 70}"
di as result "BASELINE: $shock_variable shock IRF, exposure/mitigation switched off"
di as result "{hline 70}"
run_baseline_shock_projections "`panel_ict'" "`irf_baseline'"
