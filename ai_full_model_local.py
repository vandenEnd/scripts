...
SCRIPT_VERSION = "2025-08-11-v44-hs-export-8-codes"  # bump this whenever the file changes;
...
# (quarter, gscpi_global). Which ONE of these five sheets is actually
# read depends on SHOCK_VARIABLE (see the CONFIG section below) -- only
# one of them is used per run, via fetch_shock_global().
...
SHOCK_VARIABLE = "WUI"          # "WUI", "GPR", "EPU", "TPU", or "GSCPI" --
                                 # which global shock series drives every
                                 ...
                                 # of the five -- nothing else needs to
                                 # change. All five series are read from
                                 # LOCAL_DATA_FILE (sheets "wui", "gpr",
                                 # "epu", "tpu", "gscpi" respectively); see
                                 # fetch_shock_global().
_VALID_SHOCK_VARIABLES = ("WUI", "GPR", "EPU", "TPU", "GSCPI")
...
def fetch_shock_global():
    """
    Quarterly GLOBAL shock series -- whichever of WUI, GPR, EPU, TPU, or
    GSCPI SHOCK_VARIABLE currently selects. See _SHOCK_CITATION for the
    source paper of each.

    Reads the corresponding sheet ("wui", "gpr", "epu", "tpu", or
    "gscpi" -- columns: quarter, {name}_global) of LOCAL_DATA_FILE --
    already quarterly, no further resampling needed. Regardless of which
    is selected, the sheet's own value column is renamed here to a
    single GENERIC name, "shock_level", so every downstream function
    (purify_shock(), build_panel(), etc.) is written once and works
    unchanged for any of the five shock variables -- they never need to
    know or care which one was actually chosen.
...
def purify_shock(shock_level_df, lags=AR_PURIFICATION_LAGS):
    ...
    EPU, TPU, or GSCPI -- per SHOCK_VARIABLE): regress its level on its
    own lags and take the residual as an approximate "identified shock"
    -- the part not predictable from its own recent history.

    All five candidate series are already quarterly, relatively
...
def fetch_gdp_growth_yoy():
    ...
    YEAR log growth (dgdp, used as the dgdp_lag1 control throughout this
    script -- see build_panel(), run_local_projections(), and
    run_baseline_shock_projections(); a linear time trend is also
    included as a second control alongside it).
...
def build_panel(exposure="ict"):
    ...
    The global shock (WUI, GPR, EPU, TPU, or GSCPI, per SHOCK_VARIABLE -- see the CONFIG
    ...
    if exposure == "ict":
        ...
        raise ValueError(
            "exposure must be 'ict', 'patent', 'investment', 'corr', or 'hs_export'"
        )
    ...
    # purification in purify_shock() already strips out most smooth,
    # trend-like predictability from the shock series), but some
    # residual collinearity is possible depending on SHOCK_VARIABLE and
    # sample period -- see the printed correlation diagnostic below,
    # and treat a high correlation there as a signal that b1
    # (shock_used's own coefficient) and the time_trend coefficient may
    # be poorly separately identified in that particular run.
    # ------------------------------------------------------------------
    ...
    _trend_shock_corr = panel[["time_trend", "shock_used"]].dropna().corr().iloc[0, 1]
    print(f"  [diagnostic] corr(time_trend, shock_used) = {_trend_shock_corr:.3f} "
...
def run_baseline_shock_projections(panel):
    ...
    regressors = ["shock_used", "dgdp_lag1", "time_trend"]
...
def summarize_baseline_irf(results):
    ...
    a one-unit increase in the chosen shock (WUI, GPR, EPU, TPU, or GSCPI,
...
VARIABLE_DEFINITIONS = {
    ...
    "dgdp":     "Real GDP growth, YEAR-ON-YEAR, log-difference x100 (derived from gdp_level, 4-quarter shift -- see fetch_gdp_growth_yoy()). Used only to build the dgdp_lag1 lag control; the regression's actual dependent variable (dgdp_cum_lead) is a separate, directly-constructed cumulative measure, unaffected by this.",
    ...
    "time_trend": "Explicit LINEAR TIME TREND control: an integer counter (0, 1, 2, ...) over the panel's own quarters, entity-invariant (same value for every country at a given quarter). Added because F trends upward over most of the sample for nearly every country at once (a shared AI/ICT-adoption trend), which risks the Shock*F interaction coefficient partly reflecting a generic drift in growth over calendar time rather than something specifically tied to exposure. Does NOT fully resolve this (a plain additive trend controls for drift in the LEVEL of growth, not specifically for drift in the shock's own sensitivity -- that would need a further Shock*time_trend interaction, not included here). See build_panel() for the printed corr(time_trend, shock_used) diagnostic and its interpretation.",
    ...
    "beta_b1_shock_panelavg": f"Panel-average CUMULATIVE coefficient on shock_used (b1, the {SHOCK_VARIABLE} main effect) over horizons 0..h. ICT, Corr, and hs_export specs.",
    "se_b1_shock_panelavg": "Standard error of beta_b1_shock_panelavg (Driscoll-Kraay-style)",
    "beta_b2_F_panelavg": "Panel-average CUMULATIVE coefficient on F (b2, the exposure-weight main effect) over horizons 0..h. ICT, Corr, and hs_export specs.",
    ...
    "beta_b3_dum_panelavg": "Panel-average CUMULATIVE coefficient on is_boom (b3, the Dum/regime main effect) over horizons 0..h. Corr and hs_export specs.",
    "se_b3_dum_panelavg": "Standard error of beta_b3_dum_panelavg",
    "beta_b4_shock_x_F_panelavg": f"Panel-average CUMULATIVE coefficient on shock_x_exposure (b4, Corr/hs_export specs' {SHOCK_VARIABLE}*F interaction) over horizons 0..h -- this is ALSO the mitigating effect in the reference regime (Dum=0, 'bust'); see mitigating_effect_bust_panelavg, which equals this exactly.",
    "se_b4_shock_x_F_panelavg": "Standard error of beta_b4_shock_x_F_panelavg",
    "beta_b5_shock_x_dum_panelavg": f"Panel-average CUMULATIVE coefficient on shock_x_dum (b5, {SHOCK_VARIABLE}*Dum interaction) over horizons 0..h. Corr and hs_export specs.",
    "se_b5_shock_x_dum_panelavg": "Standard error of beta_b5_shock_x_dum_panelavg",
    "beta_b6_shock_x_F_x_dum_panelavg": f"Panel-average CUMULATIVE coefficient on shock_x_exposure_x_dum (b6, the full three-way {SHOCK_VARIABLE}*F*Dum interaction) over horizons 0..h -- this is the ADDITIONAL mitigating effect specific to the boom regime, on top of b4. Corr and hs_export specs.",
...
if __name__ == "__main__":
    ...
    7. ALL RELEVANT IRF CHARTS (baseline, ICT, Corr, hs_export) are
       ...
       ratio unchanged).
    8. ALL SPECS (ICT, patent, investment, Corr, and hs_export) also
       include an explicit LINEAR TIME TREND
       control (time_trend) alongside dgdp_lag1, added specifically
       because F (the exposure state variable) trends upward over most
       of the sample for nearly every country at once -- a shared,
       roughly global AI/ICT-adoption trend. This is a genuine
       econometric concern, not a stylistic addition: without a trend
       control, the Shock*F interaction coefficient (b3/b4) could partly
       reflect "how the shock-growth relationship changed over calendar
       time in general" rather than something specifically tied to
       exposure. A PLAIN additive trend does NOT fully resolve this --
       it controls for drift in the LEVEL of growth, not specifically
       for drift in the shock's own sensitivity to it (that would need a
       further Shock*time_trend interaction, not included here). Check
       the printed corr(time_trend, shock_used) diagnostic in
       build_panel()'s output for each run: a high correlation (roughly
       above 0.5-0.6 in magnitude) signals that b1 (the shock's own main
       effect) and the time_trend coefficient may be weakly separately
       identified in that particular run, and results should be read
       with correspondingly more caution.
