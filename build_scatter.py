...
def significance_stars(x1, n1, x2, n2):
    """
    ...
    just numerically but statistically. Returns "*"/"**"/"***" for
    p<0.10/0.05/0.01 respectively, "" if not significant or if either
    n is 0 (comparison undefined, matching this project's existing
    "n/a" handling for zero-count cells elsewhere).
    """
...
def add_significance_stars(ws_summary, header_row, cluster_props, yrs_props):
    """
    Runs the three requested two-proportion z-tests and appends
    significance stars to the EXISTING header text (not replacing it)
    in the corresponding cells:
      - D{header_row} += stars for cluster "above" vs cluster "below"
      - F{header_row} += stars for yrs "full sample" vs cluster
        "full sample"
      - H{header_row} += stars for yrs "above" vs yrs "below"
    """
    d_stars = significance_stars(cluster_props["x_above"], cluster_props["n_above"],
                                  cluster_props["x_below"], cluster_props["n_below"])
    f_stars = significance_stars(yrs_props["x_full"], yrs_props["n_full"],
                                  cluster_props["x_full"], cluster_props["n_full"])
    h_stars = significance_stars(yrs_props["x_above"], yrs_props["n_above"],
                                  yrs_props["x_below"], yrs_props["n_below"])
    for col_letter, stars in (("D", d_stars), ("F", f_stars), ("H", h_stars)):
        cell = ws_summary[f"{col_letter}{header_row}"]
        if stars:
            cell.value = f"{cell.value} {stars}"
...
def panel_ols_two_way_fe(df, y_col, x_col, entity_col="country", time_col="target_year"):
    d = df[[y_col, x_col, entity_col, time_col]].dropna().copy()
    n_obs = len(d)
    ...
    x_idx = list(X.columns).index(x_col)
    beta_x, se_x = beta_hat[x_idx], se_all[x_idx]
    t_stat = beta_x / se_x
    p_value = 2 * (1 - _stats.t.cdf(abs(t_stat), dof))
    ...
    return {
        "beta": beta_x, "se": se_x, "t_stat": t_stat, "p_value": p_value,
...
# --- SECOND panel regression table, same specification, but run on
# the SHOCK-YEARS data (the "_yrs" sheets: 2020/2022/2025, Spring &
# Autumn rounds only) instead of the full sample -- using the EXACT
# same df_sorted DataFrames write_special_years_chart() built for
# those sheets (returned above as yrs_df_ict/yrs_df_hs/yrs_df_corr),
# so this regression is guaranteed to run on the identical rows the
# "_yrs" sheets display, including the 2025->2024 substitution for
# ict_share/hs_export_share where it applies.
#
# HONESTY NOTE: this sample is much smaller (2-3 distinct years, ~30-
# 40 observations) than the full-sample regression above, so it has
# far fewer degrees of freedom for the SAME number of fixed-effect
# dummies (10 country dummies + up to 2 year dummies + constant) --
# standard errors here are correspondingly less reliable, and this is
# flagged explicitly in the sheet, not just in this comment.
yrs_regression_specs = [
    ("ICT investment share (data_ict_share_yrs)", yrs_df_ict, "ict_share"),
    ("AI/ICT-related HS export share (data_hs_export_share_yrs)", yrs_df_hs, "hs_export_share"),
    ("National vs. semiconductor index correlation (data_stock_corr_yrs)",
     yrs_df_corr, "stock_semis_corr_annual"),
]

reg2_start_row = reg_header_row + len(regression_specs) + 3
ws_summary.cell(row=reg2_start_row, column=1,
                 value="Panel regressions (SHOCK YEARS ONLY): growth_surprise = const + "
                       "country FE + time FE + beta*X").font = Font(bold=True, size=12)
ws_summary.cell(
    row=reg2_start_row + 1, column=1,
    value="(same specification as above, restricted to the 2020/2022/2025 shock-years "
          "sample -- CAUTION: far fewer observations and degrees of freedom than the "
          "full-sample regression, so treat these standard errors with extra caution)"
).font = Font(italic=True, size=9)
...
for offset, (label, df_src, x_col) in enumerate(yrs_regression_specs):
    df_src = df_src.copy()
    df_src["growth_surprise"] = (df_src["realized_growth_annual_pct"]
                                  - df_src["forecast_growth_annual_pct"])
    result = panel_ols_two_way_fe(df_src, "growth_surprise", x_col)
    ...
    ws_summary.cell(row=row_idx, column=8, value=result["n_entities"])
    ws_summary.cell(row=row_idx, column=9, value=result["n_periods"])
    print(f"  Panel regression, shock years ({label}): beta={result['beta']:.4f}, "
          f"p={result['p_value']:.4f}, N={result['n_obs']}")
