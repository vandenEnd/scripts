...
for offset, (label, df_src, x_col) in enumerate(regression_specs):
    ...
    summary_table_results.setdefault(_clean_var_label(label), {})[1] = (result["beta"], result["p_value"])
...
for offset, (label, df_src, x_col) in enumerate(interaction_specs):
    ...
    print(f"  Interaction regression ({label}): beta={result['beta']:.4f} "
          ...
          f"N={result['n_obs']}")
    summary_table_results.setdefault(_clean_var_label(label), {})[2] = (gamma["beta"], gamma["p_value"])
...
for offset, (label, df_src, x_col) in enumerate(above_median_specs):
    ...
    summary_table_results.setdefault(_clean_var_label(label), {})[3] = (gamma["beta"], gamma["p_value"])
...
logit4_start_row = reg3_header_row + len(above_median_specs) + 3
ws_summary.cell(
    row=logit4_start_row, column=1,
    value="LOGIT versions of the three panel regressions above: "
          "P(growth_surprise > 0) = logit(const + country FE + time FE + beta*X "
          "[+ gamma*interaction])"
).font = Font(bold=True, size=12)
ws_summary.cell(
    row=logit4_start_row + 1, column=1,
    value="(estimates whether X makes a POSITIVE surprise MORE LIKELY, not whether "
          ...
          "tables. LSDV-style dummy-variable logit, NOT conditional/fixed-effects "
          "logit -- does not fully solve the incidental parameters problem; see "
          "panel_logit_two_way_fe()'s docstring)"
).font = Font(italic=True, size=9)
...
logit_current_row = logit4_start_row + 3
for group_label, specs, interaction_col_name in logit_spec_groups:
    ws_summary.cell(row=logit_current_row, column=1, value=group_label).font = Font(
        bold=True, size=10, italic=True)
    logit_header_row = logit_current_row + 1
    if interaction_col_name is None:
        logit_headers = ["Explanatory variable", "beta (X)", "SE(beta)", "z(beta)",
                          "p(beta)", "Pseudo R-sq", "N (obs)"]
    else:
        logit_headers = ["Explanatory variable", "beta (X)", "SE(beta)", "p(beta)",
                          f"gamma ({interaction_col_name})", "SE(gamma)", "p(gamma)",
                          "Pseudo R-sq", "N (obs)"]
    for col_idx, h in enumerate(logit_headers, start=1):
        ws_summary.cell(row=logit_header_row, column=col_idx, value=h).font = bold

    for offset, (label, df_src, x_col) in enumerate(specs):
        df_src = df_src.copy()
        df_src["growth_surprise"] = (df_src["realized_growth_annual_pct"]
                                      - df_src["forecast_growth_annual_pct"])
        row_idx = logit_header_row + 1 + offset

        if interaction_col_name is None:
            result = panel_logit_two_way_fe(df_src, "growth_surprise", x_col)
            ws_summary.cell(row=row_idx, column=1, value=label)
            ws_summary.cell(row=row_idx, column=2, value=round(result["beta"], 4))
            ws_summary.cell(row=row_idx, column=3, value=round(result["se"], 4))
            ...
            # 3) above exactly: X's own value where above median, 0
            # otherwise -- not the binary 0/1 dummy.
            df_src["above_median"] = np.where(
                df_src[x_col].isna(), np.nan,
                np.where(df_src[x_col] > median_val, df_src[x_col], 0.0))
            df_src["shock_year_dummy"] = df_src["target_year"].isin(SHOCK_YEARS_SET).astype(float)
    ...
    logit_current_row = logit_header_row + len(specs) + 3
...
model_estimator = {
    1: "OLS", 2: "OLS", 3: "OLS", 4: "Logit", 5: "Logit", 6: "Logit",
}
model_coef_kind = {
...
summary_table_start_row = logit_current_row + 2
white_fill = PatternFill(start_color="FFFFFFFF", end_color="FFFFFFFF", fill_type="solid")

ws_summary.cell(row=summary_table_start_row, column=1,
                 value="Table 1: Summary of main regression results").font = Font(
    bold=True, size=12)
...
table_display_names = [
    ("ICT investment share", "AI-ICT investment"),
    ("AI incoming investment share", "Equity investments in AI"),
    ("AI/ICT-related HS export share", "AI-ICT-related export"),
...
note_lines = [
    ...
    "(1) OLS, plain X: growth_surprise = const + country FE + time FE + beta*X.",
    "(2) OLS, X + shock-year interaction: adds gamma*(X*shock_year_dummy) to (1); "
    "gamma is the ADDITIONAL effect of X specifically during shock years.",
    # Split across two lines, same reasoning/layout as (4) below: this
    ...
    "(3) OLS, above-median value + interaction: same as (2), but X is replaced "
    ...
    # own rows 157-158) -- this one note is noticeably longer than the
    # others and was wrapping/overflowing awkwardly as a single line.
    "(4) LOGIT, plain X: same specification as (1), but P(growth_surprise > 0) "
    ...
    "(5) LOGIT, X + shock-year interaction: logit counterpart of (2).",
    "(6) LOGIT, above-median value + interaction: logit counterpart of (3).",
    # Explains the Driscoll-Kraay standard errors used in models (1),
    # (2), and (3) (the OLS specifications) -- split across two lines,
    # same reasoning as (3)/(4) above.
    "Standard errors for models (1)-(3) (OLS) are DRISCOLL-KRAAY robust, NOT plain/"
    ...
    "     expected). Models (4)-(6) (logit) still use the analytic-Hessian standard "
    "errors described in panel_logit_two_way_fe()'s own docstring, not Driscoll-Kraay.",
]
