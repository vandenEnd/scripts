...
def write_clustered_time_chart(wb, cluster_key, merged_df, cluster_col, cluster_header,
    ...
    # --- Summary block, below the data, using LIVE formulas over the
    # same growth_surprise_pct column (G) the chart itself plots --
    # reuses the contiguous below/above row ranges already computed
    # above (below_start:below_end, above_start:above_end), so no
    # COUNTIFS-on-the-cluster-column is needed -- a plain COUNTIF over
    # each block's own row range is enough.
    summary_header_row = above_end + 2  # one blank row of separation
    summary_data_row = summary_header_row + 1

    ws.cell(row=summary_header_row, column=2, value="full sample").font = bold
    ws.cell(row=summary_header_row, column=3, value="below").font = bold
    ws.cell(row=summary_header_row, column=4, value="above").font = bold

    ws.cell(row=summary_data_row, column=1, value="share positive surprises").font = bold
    full_range = f"$G${below_start}:$G${above_end}"
    below_range = f"$G${below_start}:$G${below_end}"
    above_range = f"$G${above_start}:$G${above_end}"

    cell_full = ws.cell(row=summary_data_row, column=2,
                         value=f'=COUNTIF({full_range},">0")/COUNT({full_range})')
    cell_below = ws.cell(row=summary_data_row, column=3,
                          value=f'=COUNTIF({below_range},">0")/COUNT({below_range})')
    cell_above = ws.cell(row=summary_data_row, column=4,
...
def write_special_years_chart(wb, var_key, merged_df, col, header, color):
    ...
    headers = ["country", "vintage", "vintage_round", "target_year",
               "forecast_growth_annual_pct", "realized_growth_annual_pct",
               "growth_surprise_pct", header, "cluster"]
    for col_idx, h in enumerate(headers, start=1):
    ...
    for col_idx in range(1, 10):
        ws_data.column_dimensions[chr(64 + col_idx)].width = 18
    ...
    # --- Summary block, below the data, using LIVE formulas over the
    # same growth_surprise_pct column (G) the chart plots -- now with
    # the full/below/above 3-column layout, matching the cluster
    # sheets, using the contiguous row ranges just computed above.
    summary_header_row = last_row + 2  # one blank row of separation
    summary_data_row = summary_header_row + 1
    ws_data.cell(row=summary_header_row, column=2, value="full sample").font = bold
    ws_data.cell(row=summary_header_row, column=3, value="below").font = bold
    ws_data.cell(row=summary_header_row, column=4, value="above").font = bold
    ws_data.cell(row=summary_data_row, column=1, value="share positive surprises").font = bold

    full_range = f"$G${below_start}:$G${above_end}"
    below_range = f"$G${below_start}:$G${below_end}"
    above_range = f"$G${above_start}:$G${above_end}"
    cell_full = ws_data.cell(row=summary_data_row, column=2,
                              value=f'=COUNTIF({full_range},">0")/COUNT({full_range})')
    cell_below = ws_data.cell(row=summary_data_row, column=3,
                               value=f'=COUNTIF({below_range},">0")/COUNT({below_range})')
    cell_above = ws_data.cell(row=summary_data_row, column=4,
                               value=f'=COUNTIF({above_range},">0")/COUNT({above_range})')
    for c in (cell_full, cell_below, cell_above):
        c.number_format = "0%"
