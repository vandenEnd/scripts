...
SCRIPT_VERSION = "2025-08-11-v35-GPR-moderated-regression-dk-bandwidth"  # bump this whenever the file changes;
...
def fetch_ict_investment_share():
    ...
    asset10 == "N1132G"`). Same ESA2010 asset10 vocabulary as an earlier
    version of this function used with nama_10_an6
    ...
    N1173G has a KNOWN Eurostat coverage gap for this country set in the
    related nama_10_an6 dataset (in an earlier debugging round it
    returned zero non-null values entirely there -- a real data-
    availability gap, not a wrong ESA2010 code). The same tolerance is
    kept here in case nama_10_a64_p5 has the same gap: missing N1173G
    values are treated as a zero contribution when summing rather than
    letting NaN propagate and wipe out the whole row -- N1132G alone
    still counts for a country/year that has equipment data but no
    software breakdown. N1132G and N11G are still required (dropped if
    missing); only N1173G is defaulted to zero.
...
def build_panel(exposure="ict"):
    ...
    # Single lag control, YoY GDP growth (dgdp is now year-on-year -- see
    # fetch_gdp_growth_yoy()). dgdp_lag2 has been dropped throughout this
    # script (ICT spec, Corr spec, and the baseline spec all use only
    # dgdp_lag1 now).
    panel["dgdp_lag1"] = panel.groupby("country")["dgdp"].shift(1)

    # ------------------------------------------------------------------
    # MODERATED-REGRESSION SPECIFICATION (not a "difference" spec).
    #
    ...
    #
    # b3 (Corr spec's GPR*F term) is the SAME quantity this script called
    # "gpr_x_exposure" in an earlier, difference-style version -- kept
    # under that name for continuity. Unlike that earlier version, GPR
    # (gpr_used) and F are now ALSO included as separate regressors
    # (b1, b2), not folded into an implicit baseline/intercept -- so b3
    # is now a genuine interaction-effect coefficient net of both main
    ...
    # completely (the same collinearity this script's docstrings
    # document elsewhere for the raw shock alone). Estimating b1, b3
    # (Corr spec's Dum term), and b5 as genuinely identified coefficients
    # therefore requires ENTITY fixed effects only, no time effects --
    # see run_local_projections(), which now uses entity-only FE for
    # BOTH specs (previously only the baseline spec needed this). Cost:
    # other common-across-countries time-varying confounders (ECB
    # policy, euro-area-wide demand shocks) are no longer swept out of
    # either spec's residual the way two-way FE would have done.
    # ------------------------------------------------------------------
...
def plot_baseline_irf(irf_df, title="Baseline GPR shock IRF (no exposure interaction)"):
    ...
    h = irf_df["h"]
    b = irf_df["beta_gpr_baseline"]
    se = irf_df["se_gpr_baseline"]
    ...
    ax.set_ylabel("Cumulative dGDP response per unit GPR shock (0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
def plot_irf(irf_df, title, focus_country=FOCUS_COUNTRY):
    """
    ...
    h = irf_df["h"]
    b_panel = irf_df["beta_b3_gpr_x_F_panelavg"]
    se_panel = irf_df["se_b3_gpr_x_F_panelavg"]
    ...
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Horizon h (quarters)")
    ax.set_ylabel("Cumulative GPR x F interaction coefficient (b3, 0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
def plot_regime_irf(irf_df, regime, title, focus_country=FOCUS_COUNTRY, color="#2ca02c",
    ...
    b_panel = irf_df[col_panel]
    se_panel = irf_df[se_panel_col]
    ...
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Horizon h (quarters)")
    ax.set_ylabel(f"Cumulative mitigating effect (0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
if __name__ == "__main__":
    ...
    fig_ict = plot_irf(irf_ict, "Spec A: ICT investment share exposure")
    ...
    run_local_projections() for the full identification argument). This
    is a change from an earlier version of this script, which used a
    two-way-FE "difference" spec (a single interaction term only, GPR's
    and F's own main effects folded implicitly into the intercept/time
    effects) -- b3/b4 here are genuine interaction-effect coefficients
    net of both main effects, not a "F=1 minus F=0" difference.
    ...
    5. ALL THREE specs now use entity fixed effects only, no time fixed
       ...
       euro-area-wide demand shocks at a given quarter) are no longer
       swept out of any spec's residual, unlike an earlier two-way-FE
       version of the mitigation specs. Treat every coefficient in this
    ...
    6. Standard errors are Driscoll-Kraay throughout (robust to
       ...
       _dk_bandwidth()) rather than a fixed constant -- this corrects an
       earlier version of this script, where a fixed bandwidth=4 at
       every horizon understated the serial correlation induced by the
       cumulative dependent variable's overlapping-window construction
       at longer horizons (roughly h>=4), making those horizons'
