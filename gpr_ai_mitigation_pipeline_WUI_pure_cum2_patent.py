...
INTERPRETATION: b3 (Patent spec) and b4 (Corr spec) are the mitigating
effect of AI exposure on the WUI shock's impact, net of the WUI and
...
Two versions of z (AI exposure) are estimated:
  (A) Annual AI-related patent applications, all AI fields (primary;
      ETO/CSET Country Activity Tracker, exported locally as a CSV --
...
Yahoo Finance). AI patent data (see fetch_ai_patents()) is read from a
LOCAL file ("eto_patent.csv", exported from cat.eto.tech) rather than
fetched over the network.
"""

SCRIPT_VERSION = "2025-08-11-v40-WUI-patent-local-primary"  # bump this whenever the file changes;
...
# panel_patent/panel_corr directly (they already carry wui_used = wui_shock),
...
# ^ ETO/CSET Country Activity Tracker's own direct CSV export
# (cat.eto.tech), saved locally next to this script -- this is now the
# PRIMARY source for AI patent data. An earlier version of this script
# tried fetching this same underlying CSET/ETO data from Our World in
# Data's online mirror of it -- that endpoint consistently returned HTTP
# 404 in testing (even with a full browser-like header set) despite the
# URL being independently re-confirmed correct against OWID's own chart
# page, which couldn't be further diagnosed without direct network
# access from where this script is developed. The user's own direct
# export from cat.eto.tech, saved locally, is used instead -- more
# reliable, and also the original/primary source (OWID's version was
# itself only a re-publication of this same ETO/CSET data).
...
COUNTRY_NAME_MAP = {
   ...
   # the ETO CAT export's "Country" column


# ----------------------------------------------------------------------
# 2b. ETO/CSET Country Activity Tracker pull: annual AI patent
...
def fetch_ai_patents():
    ...
    variable be patent APPLICATIONS specifically, not grants (grants lag
    applications further and would understate recent-year activity even
    more severely).
    ...
    # Diagnostic: flag likely-INCOMPLETE trailing years. ETO/CSET's own
    # documentation notes significant reporting lag for patent data --
    # the most recent 1-2 years in the confirmed real file are
    # near-zero across every one of the 7 countries combined
    # (2023: 186, 2024: 40, 2025: 0, versus e.g. 2022: 1659), consistent with
    # "not yet reported" rather than a genuine collapse in AI-patenting
    # activity. NOT dropped automatically -- surfaced here as a visible,
    # informed choice rather than a silent one; consider excluding these
    # years manually if they distort the exposure measure at the most
    # recent quarters (which annual_to_quarterly() would otherwise flat-
    # repeat across all 4 quarters of that year, including quarters that
    # postdate the true data lag).
...
def annual_to_quarterly(df_annual, value_col, quarters_index):
    ...
    if df_annual.empty:
        raise ValueError(
            ...
            "fetch (e.g. fetch_ai_patents) returned zero rows. "
...
def fetch_corr_with_semiconductor_index(countries=COUNTRIES, window=CORR_WINDOW_QUARTERS):
    ...
    / robustness specification, not a substitute for the AI-patent-
...
def build_panel(exposure="patent"):
    """
    exposure: 'patent' -> annual AI-related patent applications per
                          country, all AI fields (primary; ETO/CSET
                          Country Activity Tracker, local CSV export
                          -- see fetch_ai_patents())
    ...
    if exposure == "patent":
        ...
        raise ValueError("exposure must be 'patent' or 'corr'")
    ...
    # underlying trend (e.g. AI patent applications rising steadily across
...
def run_local_projections(panel, include_focus_interaction=True):
    ...
    - Patent spec:  wui_used (b1), F (b2), wui_x_exposure (b3, = WUI*F).
...
def summarize_irf(results, focus_country=FOCUS_COUNTRY):
    ...
    Patent spec:  b1_wui, b2_F, b3_wui_x_F
...
def plot_irf(irf_df, title, focus_country=FOCUS_COUNTRY, shock_std=1.0):
    ...
    (+/- 1.645 SE). This is the Patent spec's mitigating-effect coefficient,
...
VARIABLE_DEFINITIONS = {
    ...
    "dgdp_lag1": "dgdp (year-on-year GDP growth), lagged 1 quarter -- the ONLY lag control used throughout this script (Patent spec, Corr spec, and the baseline spec alike); a second lag (dgdp_lag2) is not used.",
    ...
    "z_raw":    "Raw AI-exposure state variable before standardization -- annual AI patent applications (Spec A) or rolling correlation of the national equity index with the semiconductor index (Spec B)",
    ...
    "wui_x_exposure": "b3 (Patent spec) / b4 (Corr spec) interaction regressor: F * wui_used. Its coefficient is the mitigating effect NET OF the WUI and F main effects (wui_used, F), which are now separately included in the regression -- not a 'difference' spec.",
    ...
    "wui_x_exposure_focus": "is_focus * wui_x_exposure -- NL's own interaction-term deviation (b3 for Patent spec, b4 for Corr spec)",
    ...
    "beta_b1_wui_panelavg": "Panel-average CUMULATIVE coefficient on wui_used (b1, the WUI main effect) over horizons 0..h. Both Patent and Corr specs.",
    "se_b1_wui_panelavg": "Standard error of beta_b1_wui_panelavg (Driscoll-Kraay-style)",
    "beta_b2_F_panelavg": "Panel-average CUMULATIVE coefficient on F (b2, the exposure-weight main effect) over horizons 0..h. Both Patent and Corr specs.",
    "se_b2_F_panelavg": "Standard error of beta_b2_F_panelavg",
    "beta_b3_wui_x_F_panelavg": "Panel-average CUMULATIVE coefficient on wui_x_exposure (b3, Patent spec's WUI*F interaction) over horizons 0..h -- the Patent spec's mitigating-effect coefficient, net of the WUI and F main effects. A POSITIVE value means exposure dampens/offsets the (typically negative) WUI shock response; negative means amplification.",
    ...
    f"beta_b3_wui_x_F_{FOCUS_COUNTRY}_total": f"{FOCUS_COUNTRY}'s total b3 (Patent spec's WUI*F interaction) coefficient (panel-average + deviation) -- {FOCUS_COUNTRY}'s own mitigating-effect coefficient",
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 70)
    print("SPEC A: AI patent applications as exposure (primary, all AI fields),")
    print("        global WUI")
    print("=" * 70)
    ...
    fig_patent = plot_irf(irf_patent, "Spec A: AI patent applications exposure", shock_std=wui_shock_std)
    ...
    print("\n" + "=" * 70)
    print("Fetching raw source series for export (GDP, AI patents, indices, WUI)")
    print("=" * 70)
    ...
    export_results_to_excel(
        panels={"Patent_exposure": panel_patent, "Corr_exposure": panel_corr},
        irfs={
    ...
    NL-SPECIFIC RESULT, SPEC A (Patent / Channel 2): in the Patent_exposure_IRF
    ...
    combined SE. A POSITIVE value means higher AI exposure dampens
    ...
    1. AI patent applications (Spec A) is annual -> quarterly step
       interpolation is crude; consider spline interpolation or a
       genuinely quarterly patent proxy (Eurostat STS) if available.
    ...
    7. ALL THREE IRF CHARTS (baseline, Patent, Corr) are rescaled to show
       ...
       underlying Excel tables (Patent_exposure_IRF, Corr_exposure_IRF,
    ...
    The Patent and Corr specs ask a follow-up question: does a country's
    AI exposure change HOW SENSITIVE its growth is to that same
    shock? This is captured by an INTERACTION term (WUI shock x exposure
    F), so the coefficients on it (b3 for Patent, b4/b6 for Corr) are not
    ...
    to a WUI shock, as a function of exposure F, is
        ...
        d(GDP growth) / d(WUI) = b1 + b3*F        (Patent spec)
    ...
    hypothesis: countries with a stronger domestic AI innovation base
    (Channel 2: technological-sophistication/innovation-capacity
    resilience -- more AI-related patenting signals a deeper stock of
    proprietary AI know-how, giving firms more homegrown options to
    adapt production, substitute for disrupted inputs, or pivot business
    models when uncertainty rises, rather than relying on off-the-shelf
    solutions from elsewhere) or more exposure to the AI-capex investment
    cycle (Channel 1: demand/terms-of-trade tailwind during a boom)
    absorb uncertainty shocks better than less-exposed peers.
    ...
    mitigating coefficient supports the "AI patenting capacity as an economic
    buffer" story; a negative one supports an "AI-related concentration as a
