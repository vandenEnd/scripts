...
INTERPRETATION: b3 (Investment spec) and b4 (Corr spec) are the mitigating
...
Two versions of z (AI exposure) are estimated:
  (A) Annual AI-related incoming investment counts, i.e. the number of
      inbound investment deals into AI-related companies (primary;
      ETO/CSET Country Activity Tracker, exported locally as a CSV --
      see fetch_ai_investment())
...
Yahoo Finance). AI investment data (see fetch_ai_investment()) is read
from a LOCAL file ("eto_inv.csv", exported from cat.eto.tech) rather
than fetched over the network.
"""

SCRIPT_VERSION = "2025-08-11-v41-WUI-investment-exposure"  # bump this whenever the file changes;
...
# (cat.eto.tech, "Investment" dataset), saved locally next to this
# script. Same local-file-first rationale as the earlier patent-based
# version of this exposure variable: fetching this kind of data from
# Our World in Data's online mirror consistently returned HTTP 404 in
# testing, so the user's own direct export, saved locally, is used
# instead.
...
# ----------------------------------------------------------------------
# 2b. ETO/CSET Country Activity Tracker pull: annual AI-related
#     incoming investment counts (local file)
# ----------------------------------------------------------------------
...
def build_panel(exposure="investment"):
    """
    exposure: 'investment' -> annual AI-related incoming investment
                              counts per country (primary; ETO/CSET
    ...
    if exposure == "investment":
        ...
        raise ValueError("exposure must be 'investment' or 'corr'")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 70)
    print("SPEC A: AI incoming investment counts as exposure (primary),")
    print("        global WUI")
    print("=" * 70)
    ...
    fig_investment = plot_irf(irf_investment, "Spec A: AI incoming investment counts exposure", shock_std=wui_shock_std)
    ...
    res_baseline = run_baseline_wui_projections(panel_investment)
    ...
    print("\n" + "=" * 70)
    print("Fetching raw source series for export (GDP, AI investment, indices, WUI)")
    print("=" * 70)
    ...
    export_results_to_excel(
        panels={"Investment_exposure": panel_investment, "Corr_exposure": panel_corr},
        irfs={
        ...
        figs={
            "Investment_exposure": fig_investment,
    ...
    NL-SPECIFIC RESULT, SPEC A (Investment / Channel 2): in the Investment_exposure_IRF
    ...
    1. AI incoming investment counts (Spec A) is annual -> quarterly step
       interpolation is crude; consider spline interpolation or a
       genuinely quarterly investment-flow proxy if one becomes
       available. Also: this is a DEAL COUNT, not a dollar-value metric
       -- it captures how many inbound AI-investment transactions
       landed in a country, not their size; a country with few but
       very large deals would show low exposure here despite
       substantial capital inflow (Raw_AI_Investment's source file also
       has disclosed/estimated VALUE metrics in millions USD, not used
       here, if that alternative framing is wanted instead).
    ...
    7. ALL THREE IRF CHARTS (baseline, Investment, Corr) are rescaled to show
       ...
       underlying Excel tables (Investment_exposure_IRF, Corr_exposure_IRF,
    ...
    The Investment and Corr specs ask a follow-up question: does a country's
    ...
    F), so the coefficients on it (b3 for Investment, b4/b6 for Corr) are not
    ...
    to a WUI shock, as a function of exposure F, is
        ...
        d(GDP growth) / d(WUI) = b1 + b3*F        (Investment spec)
    ...
    hypothesis: countries attracting more inbound AI-related investment
    activity (Channel 2: capital-attraction/investor-confidence
    resilience -- a higher count of incoming AI-investment deals signals
    that international capital views the country as a credible,
    ongoing destination for AI-related business activity; that inflow of
    outside capital, expertise, and follow-on commercial relationships
    can give the local economy more resources and outside options to
    adapt when uncertainty rises, rather than relying solely on
    domestic capacity) or more exposure to the AI-capex investment
    ...
    cushioning it. For the Investment spec specifically, this could
    ...
    mitigating coefficient supports the "AI-related capital inflow as an
    economic buffer" story; a negative one supports an "AI-investment
    dependence as a new source of fragility" story instead. Both are
    economically plausible ex ante -- that is precisely why this needs to
    be estimated rather than assumed.
    """)
