...
SCRIPT_VERSION = "2025-08-11-v33-WUI-moderated-regression-dk-bandwidth"  # bump this whenever the file changes;
...
def fetch_wui_global():
    ...
    quarterly resampling is needed (unlike the earlier GPR-based version
    of this script, which was monthly).
    ...
    with a bare KeyError. This matters because the exact row/column
    layout of this file has changed at least once already (an earlier
    version of this function assumed a "Date"-style column name, which
...
def build_panel(exposure="ict"):
    ...
    #
    # b3 (Corr spec's WUI*F term) is the SAME quantity this script called
    # "wui_x_exposure" in an earlier, difference-style version -- kept
    # under that name for continuity. Unlike that earlier version, WUI
    # (wui_used) and F are now ALSO included as separate regressors
...
def plot_baseline_irf(irf_df, title="Baseline WUI shock IRF (no exposure interaction)"):
    ...
    h = irf_df["h"]
    b = irf_df["beta_wui_baseline"]
    se = irf_df["se_wui_baseline"]
    ...
    ax.set_ylabel("Cumulative dGDP response per unit WUI shock (0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
def plot_irf(irf_df, title, focus_country=FOCUS_COUNTRY):
    ...
    h = irf_df["h"]
    b_panel = irf_df["beta_b3_wui_x_F_panelavg"]
    se_panel = irf_df["se_b3_wui_x_F_panelavg"]
    ...
    ax.axhline(0, color="black", linewidth=0.8, linestyle="--")
    ax.set_xlabel("Horizon h (quarters)")
    ax.set_ylabel("Cumulative WUI x F interaction coefficient (b3, 0..h)")
    ax.set_title(title)
    ax.legend()
    fig.tight_layout()
    return fig
...
if __name__ == "__main__":
    ...
    two-way-FE "difference" spec (a single interaction term only, WUI's
