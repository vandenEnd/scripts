...
ws_summary.cell(row=reg_start_row, column=1,
                 value="Panel regressions: growth_surprise(t) = const + country FE + "
                       "time FE + beta*X(t-1)").font = Font(bold=True, size=12)
ws_summary.cell(
...
note_lines = [
    ...
    "Timing: every explanatory variable is the country's observed value in t-1 for "
    "an outcome in year t. There is no same-year substitution or fallback; missing "
    "t-1 values are excluded from the relevant chart, test, and regression.",
