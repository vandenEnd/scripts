...
country-level AI/ICT-exposure state variable z[i,t], and GPR_shock_t is
...
effect of AI/ICT exposure on the GPR shock's impact, net of the GPR and
...
effect of GPR*F is b4 alone in the bust regime (Dum=0) and b4+b6 in the
...
Run:  pip install pandas numpy requests statsmodels linearmodels yfinance openpyxl matplotlib Pillow
      python gpr_ai_mitigation_pipeline_GPR_panel_moderated.py

Network access required (Eurostat API, matteoiacoviello.com, Yahoo Finance).
"""

SCRIPT_VERSION = "2025-08-11-v38-GPR-time-trend-control"  # bump this whenever the file changes;
...
print(f"[gpr_ai_mitigation_pipeline_GPR_panel_moderated.py version {SCRIPT_VERSION}]")
...
def compute_gpr_shock_std():
    ...
    plot_regime_irf()) from "response per 1 raw unit of gpr_used" (an
    arbitrary scale -- gpr_used is an AR(2) residual, not a naturally
    interpretable unit) into "response per 1-stdev GPR shock" (the
...
# panel_ict/panel_corr directly (they already carry gpr_used = gpr_shock),
...
def build_panel(exposure="ict"):
    ...
    #
    # ICT spec:  GDPgrowth = b0 + b1*GPR + b2*F + b3*(GPR*F) + controls
...
def run_local_projections(panel, include_focus_interaction=True):
    ...
    - ICT spec:  gpr_used (b1), F (b2), gpr_x_exposure (b3, = GPR*F).
...
def summarize_irf(results, focus_country=FOCUS_COUNTRY):
    ...
    ICT spec:  b1_gpr, b2_F, b3_gpr_x_F
...
VARIABLE_DEFINITIONS = {
    ...
    "gpr_x_exposure": "b3 (ICT spec) / b4 (Corr spec) interaction regressor: F * gpr_used. Its coefficient is the mitigating effect NET OF the GPR and F main effects (gpr_used, F), which are now separately included in the regression -- not a 'difference' spec.",
    ...
    "gpr_x_exposure_focus": "is_focus * gpr_x_exposure -- NL's own interaction-term deviation (b3 for ICT spec, b4 for Corr spec)",
    ...
    "beta_b1_gpr_panelavg": "Panel-average CUMULATIVE coefficient on gpr_used (b1, the GPR main effect) over horizons 0..h. Both ICT and Corr specs.",
    ...
    f"beta_b3_gpr_x_F_{FOCUS_COUNTRY}_total": f"{FOCUS_COUNTRY}'s total b3 (ICT spec's GPR*F interaction) coefficient (panel-average + deviation) -- {FOCUS_COUNTRY}'s own mitigating-effect coefficient",
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 70)
    print("SPEC A: ICT investment share as exposure (primary), global GPR")
    print("=" * 70)
    ...
    fig_ict = plot_irf(irf_ict, "Spec A: ICT investment share exposure", shock_std=gpr_shock_std)
    ...
    fig_corr_boom = plot_regime_irf(
        ...
        color="#2ca02c", color_nl="#98df8a", shock_std=gpr_shock_std,
    )
    fig_corr_bust = plot_regime_irf(
        ...
        color="#d62728", color_nl="#ff9896", shock_std=gpr_shock_std,
    )
    ...
    res_baseline = run_baseline_gpr_projections(panel_ict)
    irf_baseline = summarize_baseline_irf(res_baseline)
    print(irf_baseline.to_string())
    fig_baseline = plot_baseline_irf(irf_baseline, title="Baseline GPR IRF (AR-purified shock, no exposure)",
                                      shock_std=gpr_shock_std)

    print("\n" + "=" * 70)
    print("Fetching raw source series for export (GDP, GFCF, indices, GPR)")
    print("=" * 70)
    ...
    sheet, look at beta_b3_gpr_x_F_{FOCUS_COUNTRY}_total and
    se_b3_gpr_x_F_{FOCUS_COUNTRY}_total -- NL's total b3 coefficient
    ...
    the GPR shock's impact on growth (consistent with firm-level
    resilience); NEGATIVE means exposure amplifies it. beta_b1_gpr_* and
    beta_b2_F_* report the GPR and F main effects themselves, now
    ...
    5. ALL THREE specs use entity fixed effects only, no time fixed
       effects (required by including GPR/Dum main effects -- see the
    ...
    7. ALL THREE IRF CHARTS (baseline, ICT, Corr) are rescaled to show
       the cumulative dGDP response per 1-STANDARD-DEVIATION GPR shock
       (gpr_shock_std, printed above -- see compute_gpr_shock_std()),
       NOT per 1 raw unit of gpr_used -- gpr_used is an AR(2) residual,
       ...
       Baseline_GPR_IRF) still report the raw per-unit coefficients
       exactly as estimated, so any further hand-calculation from the
       tables needs to multiply by gpr_shock_std manually to match what
    ...
    The baseline spec asks a simple question: does a GPR (geopolitical
    risk) shock move GDP growth at all, on average across the panel?
    beta_gpr_baseline is that average marginal response -- typically
    expected to be NEGATIVE (higher geopolitical risk depresses growth),
    ...
    shock? This is captured by an INTERACTION term (GPR shock x exposure
    ...
    to a GPR shock, as a function of exposure F, is

        d(GDP growth) / d(GPR) = b1 + b3*F        (ICT spec)
        d(GDP growth) / d(GPR) = b1 + b4 + b6*Dum  (Corr spec, at F=1)

    A POSITIVE b3 (or b4/b6) means: as exposure rises, the growth
    response to a geopolitical-risk shock becomes LESS negative -- i.e.
    ...
    terms-of-trade tailwind during a boom) absorb geopolitical-risk
    ...
    chain) makes a country's growth MORE fragile to geopolitical-risk
    shocks, not less, especially when that same AI-capex cycle is itself
    in a downturn (bust).

    In short: this script is not testing "is geopolitical risk bad for
    growth" (the baseline spec) -- it is testing "does AI/ICT exposure
    make a country's growth more or less fragile to geopolitical risk,"
    which is a claim about a SECOND DERIVATIVE (how the shock's own
    effect changes with exposure), not the shock's effect itself. A
